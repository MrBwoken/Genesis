--[[
FDless Reanimate from Patchma Hub V45
...
--note to self: Do NOT call FDless MyWorld Reanimate 💀💀💀
...
--the script doesnt have to read global varibles every time to get them
--why not have them saved in local varibles for faster access times
local next=next
local pcall=pcall
local xpcall=xpcall
local type=type
...
local accessorylimbs={
	...
	{meshid="4094864753",textureid="4094881938",C0=angles(0,0,rad(-90)),Name="Left Leg"},
	{meshid="4154474745",textureid="4154474807",C0=angles(0,0,rad(90)),Name="Right Leg"},
	{meshid="4489232754",textureid="4489233876",C0=angles(0,0,rad(-90)),Name="Left Arm"},
	{meshid="4324138105",textureid="4391374782",C0=angles(0,0,rad(90)),Name="Right Arm"},
}
...
local autorefit=autorefit~=true
...
if permadeath then
    if replicatesignal then
        ...
        sg:SetCore("SendNotification",{Title="Permadeath",Text="Your executor does not support permadeath! Please see #supported-executors channel."})
    end
else
    replicatesignal=nil
    pdloadedtime=nil
...
local reanimate=function()
	...
	local predictionfling=function(target)
		...
		if highlightflingtargets then
			...
			if not pcall(insSet,h,"Parent",FindFirstChildOfClass(game,"CoreGui")) then
				insSet(h,"Parent",pg)
			end
...
return {reanimate = reanimate, stopreanimate = stopreanimate, gp = gp, rs = rs, insSet = insSet, insGet = insGet, cfMul = cfMul, mouse = mouse, isReanimated=function() return c end}
